  ...
  return (
    <section className="w-full flex xl:flex-row flex-col justify-center min-h-screen gap-10 max-container" id="home">
      ...
      bg-primary bg-hero bg-cover bg-center">
        <img src={bigshoe} alt="bigsho1" width={610} height={500}
          className="object-contain relative z-10" />
        <div className="flex sm:gap-6 gap-4 absolute -bottom-[5%] sm:let-[10%] max-sm:px-6">
          {shoes.map((shoe) => (
            <div key={shoe}>
              <ShoeCard imagURL={shoe} changeShow={(shoe) => setbigshoe(shoe)}
                bigShoe={bigshoe} />
            </div>
          ))}
        </div>
